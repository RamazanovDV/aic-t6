...
4. **Sticky Notes** - Из диалога извлекаются ключевые факты (имя, цели, предпочтения, договорённости), которые отправляются вместе с N последними сообщениями

Настройка через UI: кнопка "Настройки оптимизации контекста" в диалоге.

### Sticky Notes

При включённом режиме **Sticky Notes**:
- Модель получает инструкцию из `FACTS_EXTRACTION.md` возвращать факты в JSON-блоке
- После каждого ответа факты извлекаются и сохраняются в сессию
- JSON-блок удаляется из сообщения для пользователя
- На следующий запрос модели отправляются: текущие факты + N последних сообщений
- В debug режиме видны поля `facts` (распарсенный dict) и `raw_facts` (оригинальный JSON от модели)
...
- Кнопка 🔍 на сообщениях от модели для просмотра debug данных (включая facts и raw_facts для sticky_notes)
